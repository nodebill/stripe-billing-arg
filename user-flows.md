...
3. The user can narrow the list with invoice `status` and UTC `date_from` / `date_to` filters over invoice creation time.
4. The table shows invoice ID, customer link, subscription ID, status, collection method, amount, timing, and delivery state.
5. The user can open the shared invoice detail dialog from the global list without leaving `/billing/invoices`.
6. If more than 200 invoices exist, the user can load more results from the same screen without losing the applied filters.
